...
contract Lottery is Ownable {
    ...
    {
        uint256 winerRatio = (fin[_level].received_entry.mul(100)).div(fin[_level].total_entry);
        uint256 newwinner = (winerRatio * fin[_level].winners);
        if (newwinner < 100) {
            uint256 _amount = ((newwinner).mul(fin[_level].winningamount)).div(100);
            uint256 _amounteverywinner = (_amount).div(fin[_level].winners);
            return (winerRatio, newwinner, _amount, _amounteverywinner);
        } else if (newwinner >= (fin[_level].winners).mul(100)) {
            uint256 _amount = fin[_level].winners.mul(fin[_level].winningamount);
            uint256 _amounteverywinner = (_amount).div(fin[_level].winners);
            return (winerRatio, newwinner, _amount, _amounteverywinner);
        } else {
            uint256 _newwinner = (newwinner).div(100);
            uint256 _amount = _newwinner.mul(fin[_level].winningamount);
            uint256 _amounteverywinner = (_amount).div(_newwinner);
    ...
    function selectWinner(uint256 _level) public onlyOwner {
        ...
        uint256 winerRatio = (fin[_level].received_entry.mul(100)).div(fin[_level].total_entry);
        uint256 newwinner = (winerRatio.mul(fin[_level].winners));
        ...
        if (newwinner < 100) {
            ...
            for (uint256 i; i < fin[_level].winners; i++) {
               ...
               winning_amount[winner] =winning_amount[winner].add(_amounteverywinner);
                total_reward = total_reward.add(_amounteverywinner);
                emit WinnerSelected(winner, _amounteverywinner);
                uint256 lastindex = (fin[_level].players.length).sub(1);
        ...
        if (_level == 1 || _level == 2 || _level == 3) {
            fin[_level].time = block.timestamp.add(7 days);
        } else if (_level == 4) {
            fin[_level].time = block.timestamp.add(14 days);
        } else if (_level == 5 || _level == 6) {
            fin[_level].time = block.timestamp.add(21 days);
        } else if (_level == 7) {
            fin[_level].time = block.timestamp.add(30 days);
        } else if (_level == 8) {
            fin[_level].time = block.timestamp.add(35 days);
        } else if (_level == 9) {
            fin[_level].time = block.timestamp.add(50 days);
        } else if (_level == 10) {
            fin[_level].time = block.timestamp.add(60 days);
        } else if (_level == 11) {
            fin[_level].time = block.timestamp.add(90 days) ;
        } else if (_level == 12) {
            fin[_level].time = block.timestamp.add(180 days);
        } else if (_level == 13) {
            fin[_level].time = block.timestamp.add(270 days);
        } else if (_level == 14) {
            fin[_level].time = block.timestamp.add(300 days);
        } else if (_level == 15) {
            fin[_level].time = block.timestamp.add(360 days);
        } else {
            fin[_level].time = block.timestamp.add(365 days);
        }
